...
If you remember in the previous session we stated that every column in a data frame is a variable and it is good practice to not have spaces within variable names.
...
We can clearn the names of our dataset with the `clean_names()` function as shown below.
...
```{r}
# We can both use regular sequences with ":" and spefic numbers.
...
The code below creates a new column  called `outstanding` into our dataset. This new column is filled with `TRUE/FALSE` when the condition specified matches `overall_effectiness == `.
...
**Create a new column in the school dataset, called `nursery`. Using the `phase_of_education` when it is equal to nursery *and* the `early_years_provision` is less than 5 years** 
