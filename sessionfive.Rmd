...
To be able to calculate our mean we need to remove them from the calculation and we can do this by adding an argument within the function: `na.rm = TRUE`.

**na.rm = TRUE**means remove missing values (na stands for missing values and rm is remove), this removes the missing values before calculating the mean, not removing the values from the dataset.
