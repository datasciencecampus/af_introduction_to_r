...
Typing the code above in your console, will result in a **"+"**.
...
You will get an error here stating,

>**Error in Print("R is the best") : could not find function "Print"**

We will go into details about function later on in the course for now just think of them as like commands they basically take an input do something and produce an output. R is termed a `functional programming language`, we will be using these throughout the course, you will be able to spot these as they will have a name and some brackets, e.g `print()` 
...
>**Functions in R can be spotted by a word followed by a set of parentheses, like so: word().More often than not, the word is a verb, such as round(), suggesting that we’re about to perform an action.**

You can specify what you the want function to exactly do with we call parameter and argument. This is specified within the brackets of the function. We talk more about parameters and arguments in chapter 6, **A parameter is a name or a label for an argument that follows it**. e.g in the round function below our parameter is `digits` and our `arguement` is 2.
...
Where `my_first_variable` is the variable name and `value` is the data being assigned to `my_first_variable`. 

```{r}
#To assign a variable
my_first_variable <- 25
```
...
After the assignment, to access or use the value we simply reference `my_first_variable` in our code. 
```{r}
#To display the variable
my_first_variable
```

We can use in mathematical operations.


```{r}
#Multiplcation using our variable name
my_first_variable * 3
```
...
For example: `rm(my_first_variable)` 

```{r}
rm(my_first_variable)
```
...
1. Store as a variable the **sum of 1 + 3**, call it **myVariable** 

2. and another variable **2 - 4**, call it **another_variable**.
...
**Variables can not start with a number, symbol  or a reserved word like `TRUE`.**
