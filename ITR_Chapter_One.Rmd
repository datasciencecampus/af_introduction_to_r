...
You can also ammend fonts here.
...
1. Try and type the expression below and hit **`Enter`**. Make sure you type one line at a time.
...
You can specify what you the want a function to exactly do with what we call parameter and argument. This is specified within the brackets of the function. We  will talk more about parameters and arguments in chapter 6, 
...
Numbers are grouped together into numerics you may see;
...
These different types exist in R for operations behinf the scenes.We need to worry about them too much at the moment.
...
When reading that code say `“my_first_variable  gets the value 25”` in your head.
...
RStudio automagically surrounds `<-` with spaces, which is a good code formatting practice. 
...
typeof(my_second_variable)

```
```{r}

my_second_variable <- "What?! A CHARACTER?!"
...
I recommend snake_case where you separate lowercase words with `_`. See examples below;
...
The variable printed `my_varIable`is no the `my_variable`
