...
is a range of options in R to load Excel data files directly. This exercise
builds on the material and techniques presented earlier. We will explore how to
load a file from a url, read different sheets, write new excel files, and
...
We will first access the data. We have previously shown how to use the function
`read_excel()` from package [readxl](https://readxl.tidyverse.org/) to load data
from a local file. In this case we will get the Consumer Price Index data
...
```{r load_from_www, warning=FALSE, message=FALSE, eval=FALSE}
library(httr) # For loading the data

# URL from which we want to download (full path to file):
...
# Download the file to your Home folder. Change the "path" string as required.
get_msg <- httr::GET(cpi_excel_url, write_disk(path = "~/consumerpriceinflationdetailedreferencetables.xls"))
```
...
```{r load_from_disk}
cpi_excel <- "~/consumerpriceinflationdetailedreferencetables.xls"
```
You should then be able to continue with the examples/code below.

> Throughout this exercise, we strongly encourage you to open and explore the
Excel data file you have downloaded, either in Excel or another appropriate
spreadsheet app. This will help you understand the structure of the data before
you load and work with it in R.
...
simplicity here. We'll cover additional ways to handle this at a later on in
this exercise.  
By printing the resulting table to screen (last line below), you
can see that the top lines of the loaded data table now actually  has the
correct numbers/values.
...
1. You can use the "range" argument in `read_excel()` as shown above to specify
which specific rows and columns to read. Also consider using the further
"col_names" argument to avoid R setting explicit column headers (you just need
this data to process into new headers, anyway).
2. Have a look at the `paste()` help page and specifically the examples used.
3. Remember you simply give a table new column names by assigning `names(table)`
(or `col_names(table)`; those functions are equivalent) a new vector of values.
Note that that new vector will need to be the same length as the number of
columns.
...
It's helpful to have a look at the data you've just loaded:
```{r exercise4_1_a}
cpi_table13_headers
```

Note that you now have a small table with two rows; one with a series of years,
and one with months in each of those years. It's really the combination of those
two that should form the column headers.  

We can extract the two rows of the dataframe as separate vectors, just to make
our lives easier. In the example below we use dataframe indexing (e.g.
`table[row, column`]) to select each individual row and store them as separate
vectors.  
...
between the two character strings. To avoid having these in column names (which
is not good practice), set the argument "sep" to "_". This uses an underscore as
a separator between the two string values, instead of a space.  
Again, we print the result so we can see what we've done:
```{r exercise4_2}
...
already done so. You'll need to use function `write.xlsx()`.
2. Have a look at
...
> Please note that you'll need to specify an appropriate path to store the data -
the specific path given below may not work.
