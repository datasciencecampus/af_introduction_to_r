...
It is likely that some data will be presented in excel sheets (`.xls` or `.xlsx`) 
rather than in the `.csv` format. These will be tables that are probably focused 
on being "human legible" so they are not the easiest to work with using R. Excel 
files also have the added complexity of having multiple sheets. 

Here we will explore how to load a files from a url, handle excel files, read 
different sheets, and write new excel files. We will provide example answers but 
it is encouraged you come up with your own examples.
...
We will first access the data. In this case we will get the Consumer Price 
Inflation data from the Office for National Statistics website. We will use 
the package `httr` see the documentation [here](https://httr.r-lib.org/).
