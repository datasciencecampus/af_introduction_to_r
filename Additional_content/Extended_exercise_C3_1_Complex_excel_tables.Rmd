...
Unfortunately, the <mark>readxl</mark> package we have used so far does not have
functionality to _write_ Excel files. We can however do this using either the
<mark>writexl</mark> package (which can write in both `.xls` and `.xlsx` format)
or the <mark>openxlsx</mark> package. We here use the latter, and write to
`.xlsx` format (as this is a newer file extension version).
...
1. Remember to install and load package <mark>openxlsx</mark> if you haven't
already done so. You'll need to use function `write.xlsx()`.
2. Have a look at the documentation for `write.xlsx()` to find out how you can
write specific dataframes (or tibbles) to specific sheets. Basically, instead of
giving `write.xlsx()` a single table to write, you can give it a named list of
tables. See <mark>Section XXX of Chapter XXX</mark> for more information on how
to deal with lists.
