...
package [dplyr](https://dplyr.tidyverse.org/index.html) or
[tidyverse](</mark>)). Please consider what each of these functions does so you
can gain the information you are looking for.
...
are capitalised and contain dots. It is good practice to decapitalise names and
replace characters like dots (or spaces) with underscores. While you can do this
manually, the function `clean_names()` in the
...
names(cpi_indices)
```
Note that in addition to replacing the dots in the column name with an
...
This task is somewhat more complex but only for two reasons. First, we now
already have a "name" column in the table we are joining to, and second, the
index we want to join on is named differently in the table we are joining _to_
(that is, in this case, we want to join using "short_code" in `CPI indices
...
renaming the "name" column to "name2" (this could be anything, as long as it's
not "name" - this avoids `left_join()` adding subscripts to our column names).
...
the right.
Finally, we use `relocate()` as before to move the joined column ("name2") to a
different place in the table; and we show the table limiting by the first five
...
need (`name2` and `short_code` is essentially repeated a number of times, so it
maybe easiest to subset and remove the repeated data (see e.g. `unique()`)
...
either using the short code value to index on, or the category name.
``` {r exercise3.3}
cpi_indices <- cpi_indices %>% 
  left_join(n_scode %>% 
              select(short_code, n),
...
4. Join the mean indices for these two new categories for the last two years
from your new table, into the original CPI data, but make sure you retain all
...
higher level categories (`name2`) and a second column that re-groups these
...
joining between data sets. It should be stressed that this is a _huge_ topic to
cover, and that there are infinite numbers of specific problems, and particurly
