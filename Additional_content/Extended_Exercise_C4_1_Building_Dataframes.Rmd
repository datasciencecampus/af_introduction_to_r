...
We also notice that the types can actually just be read straight from the environment pane in RStudio.

# Create dataframe {.tabset}

Now that we've created our data vectors, let's join them together into a
single data object. This can be a dataframe or a tibble, whichever you're more
comfortable with. Remember to name it something appropriate, and make sure all the column names make sense too.

## {-}

## Hint {-}

The `data.frame()` function is used to create dataframes, and the `tibble::tibble()` fuction for tibles. Remember to first load the `tibble` library for this one.
