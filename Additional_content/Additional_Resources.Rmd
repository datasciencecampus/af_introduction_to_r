...
library(kableExtra)
# If you get the error object ‘faststack’ is not exported by 'namespace:fastmap' you must also install or update faststack
...
bikes <- read_csv("../Data/bikes_data_set.csv")

revenue_dataset <- read_csv("../Data/revenue_data_set.csv")
...
class_survived_frequency <- table(titanic$survived, titanic$sex_of_passenger)
...
# Converting Coulumn types

Sometimes columns will read in with an incorrect data type.

This is usually because of an issue with the contents of a column. This can be something as simple as commas in numbers or missing values.

## Forcing Column types on reading in data

When reading in data using `readr::read_csv` and `readxl::read_xls` you can specify the column types.

Here the entries with missing values (* and . ) will show as parsing failures but will become `n/a` missing values.

Here they are true missing values - and replacing them with `n/a` isn't an issue - but this should be done with caution. A value of `1,0000` or `two` may cause a column to be read in as charachter but is still valid data that should be fixed before converting.


```{r}

titanic_2 <- readr::read_csv("../Data/titanic.csv",
                             col_types = list(`Age of passenger` = col_double()))
```

## What are my missing values?

Using `dplyr::count()` on a column can help to identify missing values.

```{r}
# Read in with missing values

titanic_3 <- readr::read_csv("../Data/titanic.csv")

titanic_3 %>% 
  dplyr::count(`Age of passenger`)
```


## Converting column types after reading in

This is ideally a 3 step procedure

* Identify the error
* Fix the error
* Convert the column

As seen above the issue in the `Age of passenger` column is `*` and `.` 

```{r}
# Fix error

titanic_3 <- titanic_3 %>% 
            dplyr::mutate(`Age of passenger` = dplyr::na_if(`Age of passenger`, "*")) %>%
            dplyr::mutate(`Age of passenger` = dplyr::na_if(`Age of passenger`, "."))

# Convert column

titanic_3$`Age of passenger` <- as.numeric(titanic_3$`Age of passenger`)

```


Note that if you do convert a column to numeric without fixing the issues it will just make any character values missing values. This might be desirable but could also cause issues in your data.
