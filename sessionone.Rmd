...
R is an interpreted language. This means it doesn't need to be converted into separate files your machine can read in order to be run, the machine interprets it for you. 
...
This means you can type code and save it for later, and run it all at once instead of a line at a time. 
...
#### **Questions**{-}
Create a new script, choose a sensible name (e.g. intro.R). The ".R" denotes that it is a R file.
...
To run the code this time you need to select the section you want to run and then press **CTRL + ENTER.**
...
Shows all the **objects** that you have created or are working with.
...
We assign variables using the assignemt operator ` <- `, (the shortcut is **ALT + MINUS)**.
...
You may also want to remove a variable, in which case you need the `rm()` command. You put the nameof the variable that you want to remove within the parentheses. 
...
Variables can not contain a space within them such as `the number`.
...
‘**need** to be surrounded by quotation marks **that is** how R understands **it is** a character’

**Numeric - num (sometimes int/dbl)**

Numbers in R are grouped together. You may also see 

* int for integer (whole numbers with no decimal place, they are denoted in R by an L at the end of the number).

* dbl for double point floating number (a decimal).
...
These are also known as boolians. 
...
**Simplified, data types are: numbers, text and TRUE/FALSE values**.
```{r, eval=FALSE}
...
We could also convert values from one type to another, suppose we wanted to convert **a** (above) to numeric.
