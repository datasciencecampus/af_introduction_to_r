...
The `janitor` package offers many functions used to manipulate data, for example removing empty rows and columns, finding duplicates within a dataframe. In this session we will use the library to to clean our data set names.
...
mutate_example <- dplyr::mutate(.data=titanic, 
       sex_of_passenger = 
         stringr::str_to_upper(sex_of_passenger))
...
recode_example <- dplyr::mutate(.data=titanic,
                survived_logical = 
                recode(survived, '1' = TRUE, '0' = FALSE))
```
...
marvel_full_join <- dplyr::full_join(dataframe, dataframe,
                                     by = column)
...
marvel_left_join <- dplyr::left_join(
                          dataframe, 
                          dataframe, by = c(columns))
