...
>**Successful installation should finish with message as below, the last box, the rest are warnings which you will get sometimes**
...
We’re not using `janitor` in this sessions so we are not going to load it yet.
...
* `functionName(argument1 = value, argument2 = value, and so on)`
...
>**I would encourage you to specify the arguments and parameters as it makes your code easy to read**
...
**In windows file paths are specified using back slashes,In R a backslash already has a meaning, this means we use a forward slash or two back slashes instead.**
...
```{r, eval=FALSE}
"C:/My_RStudio/Workspace"/Intro_to_R_/Data/titanic.csv"
```

This is clear and explicit about where the data is stored. However if you were to use this link you would need to change elements of it – for example you might have your `Intro_to_R` in separate folder.

I can use what’s called a relative path.
...
utils::str(titanic) # Structure of the table
...
base::names(titanic) # Names of columns
...
base::unique(titanic$Pclass) #Tells us the unique elements of a variable.
