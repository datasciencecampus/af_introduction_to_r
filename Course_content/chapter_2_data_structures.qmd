...
![](../Images/combine_function.PNG){fig-alt="The help documentation for the c() function in R."}
...
We see that this is a character vector, a.k.a, a vector containing character data. This is because each element contained within the vector are character data. 
...
It is the most commonly used basic data structure in R with a lot of useful properties.
...
2. Use the c() function to create a new vector with your vectors as inputs. This is combines the smaller vectors into a larger vector, further showcasing their versatility.
...
We end up with a character vector as opposed to an error for incompatible types (numeric and character). As vectors are supposed to be of the same data type, R will convert values so that they are all the same datatype. This is called **coercion**.

This works on a complex hierarchy, as some datatypes are more all encompassing than others. For example:
...
The operations scale to a larger sized vector, rather than the individual elements we have worked with thus far. 
...
As an example, take the favourite_movies vector from the earlier exercise.
...
Run the following lines of code one at a time and try to determine what they do.
...
From here, we are going to use special type of data frame called a **tibble**.
...
We actually require a package to be loaded in in order to create tibbles and we have yet to cover this. As such, we will use a dataset that is part of R out of the box.

The palmerpenguins data contains size measurements for three penguin species observed on three islands in the Palmer Archipelago, Antarctica.

>**Don't worry about running the code here as we will see tibbles in Chapter 3. Just observe the visual difference between them and Data Frames for now.**
...
So much more is shown in a print out of a tibble, even some extra information is written when all columns cannot be shown, their types etc. 
