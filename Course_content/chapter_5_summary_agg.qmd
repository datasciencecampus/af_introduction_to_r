...
In this section, we will explore some single variable
descriptive statistics.


## Descriptive Statistics for Numerical Data

Let's start with the numerical data, because that's the easiest to work with. 

The **summarise()** verb allows us to create one or more numeric variables summarising the variables we specify. This will give one value in a column we can name ourelves.
...
```{r}
# Calculating the mean fare paid by passengers on the titanic
...
By default, this will throw most of the summary functions (to let you know that you need to sort them out), but there is an optional parameter we can turn on, called "**na.rm = TRUE**".
...
>**name = summary(column, na.rm = TRUE)**
...
titanic_data |>  
    summarise(median_fare = median(fare, na.rm = TRUE),
              ...
              median_age = median(age_of_passenger, 
                                  na.rm = TRUE),    
              mean_age = mean(age_of_passenger, 
                              na.rm = TRUE),        
...
We could restructure it using functions from the **tidyr** package such as:
* **pivot_wider()**, 
* **separate()** 
* **pivot_longer()**. 

This is more advanced however and is above the scope of this chapter. Check the additional materials chapter for more details. 
...
If we do not supply this value, we return:

* The minimum value (0th percentile)
* The lower quartile (25th percentile)
* The median/middle quartile (50th percentile)
* The upper quartile (75th percentile) 
* The maximum value (100th percentile)

but this behaviour, and also returning a sequence of quartiles, has been deprecated in newer versions of tidyverse.

This means that it is no longer supported and will be removed in the future. 

```{r}
# Computing the lower quartile

titanic_data |> 
    summarise(fare_lower = quantile(fare,
                                        probs = 0.25,
                                        na.rm = TRUE))
```
...
titanic_data |> 
    summarise(fare_upper = quantile(fare,
                                        probs = 0.75,
                                        na.rm = TRUE))
```
...
* **distinct()** - Which will list all the unique values in a given column. 
...
Note that **sort = TRUr** sorts the resulting output in descending order.
...
2. Use a summary function for textual data to determine the number of men and women in the dataset.
...
titanic_data |> 
    summarise(youngest_age = min(age_of_passenger,
                                 na.rm = TRUE),
              oldest_age = max(age_of_passenger,
                               na.rm = TRUE))
...
* Split - The tibble is divided into a set of smaller tibbles, based on the grouping variable.
    For example, if we use a variable with 3 unique categories, 3 smaller sub-tibbles are created.
...
![](../Images/group_by_image.jpg){fig-alt="Tibble grouped into 3, mean applied and combined back together.}
...
Order does not **inherently** matter here as it just determines what the final groupings look like, we will still get the same 12 each time.
...
* Meticulously clean them with numerous imputation methods (fill them in).
...
This returns a vector of TRUEs and FALSE, one for each cell in the table, for whether they are missing or not. 

We can then sum() these up to find the total number of missing values in that column. 

Remember, TRUE and FALSE have numerical representation behind the scenes.
...
titanic_data <- titanic_data |> 
    mutate(embarked = replace_na(embarked,
                                 replace = "S")) 
...
We risk lowering variance across our dataset and overall extrapolation by removing them, but if analysis determines that there is no easy way to impute them, dropping them might be the only option.
...
As such, we would likely remove this column entirely, as dropping rows with null values would remove around 75% of the dataset.
...
* Complete any of the R courses that this is a pre-requisite for on the [Learning Hub](https://learninghub.ons.gov.uk/local/catalogue/index.php), such as:
