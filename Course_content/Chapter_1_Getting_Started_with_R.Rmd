...
You can specify what you the want a function to exactly do with what we call an argument. This is specified within the brackets of the function. We  will talk more about arguments in chapter 6, 

The inputs of function is called an argument and what we specify is called a value .

**A argument is a name or a label for a value that follows it**. e.g in the round function below our `arguement` is `digits` and our `value` is 2.
...
R has a lot of flexibility when it comes to variable assignment.

We assign variables using the assignemt operator `<-`.

>**Short cut keys Alt + Minus**

RStudio automatically surrounds `<-` with spaces, which is a good code formatting practice.

For example below; where `weight_kg` is the variable name and `value` being assigned to it is `60`. 
...
Now am reassigning height which was assigned to 5 to 7.
