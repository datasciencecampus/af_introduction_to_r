...
>**TIP - Comments can also be used to prevent R from running code**
...
```{r example showing arguements within functions}
# Using built in function round()
...
Note that while they can be abbreviated to `T` and `F`, this is not regarded as best practise.
...
Does 1 not equal 2?
```{r compariosn operator third example}
1 != 2
```
...
We can use our variables in mathematical operations. Here we are using `weight_kg` to calculate `weight_lb`.
...
We have more detailed examples and information on good variables names in our other course **Best Practice in Programming -  Clean Code**
...
1. Using the distance and speed values from the exercise above, calculate the speed as below.
