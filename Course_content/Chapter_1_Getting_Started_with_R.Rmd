...
The input of function is called a parameter and what we specify is called an argument.

**A parameter is a name or a label for an argument that follows it**. e.g in the round function below our parameter is `digits` and our argument is `2`.
...
We can use the variable name in mathematical operations. Here we are using `weight_kg` to calculate `weight_lb`.
...
>**`<-` is the most commonly used assignment operator in R and therefore the one we will stick to(the short cut is ALT + MINUS).**

RStudio automatically surrounds `<-` with spaces, which is a good code formatting practice. 

R will also accept using an equals sign, or an inverted arrow to assign values, but these are considered bad practice and are not recommended:

* `weight_kg = 40`

* `40 -> weight_kg`

In R it is convention to use `<-` to assign variables and keep `=` reserved to passing arguments into a function. We will be sticking to this convention. 
