...
We input the data we want to sort, and what we are going to sort it by specifying `desc`.
...
base::names(titanic_select_exercise)
```
...
base::unique(fare_more_than_200$fare)
...
For example creating the family size of the person.
...
* Minimize the need for local variables.
...
Another common way in which datasets are combined is by binding them. Unlike the join function, the binding functions do not try to match by a variable, but instead simply combine datasets. If the datasets don’t match by the appropriate dimensions, one obtains an error.
...
We can use `bind_rows()` function, which will simply merge two data frames into one.
