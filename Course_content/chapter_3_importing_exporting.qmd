...
Where we are currently at level 2 on the course content branch, so must follow the path back to the data folder and hence the dataset. 
...
> **Where there is the a ".", "*", NULL value or a blank space, class is it as a missing value.**
...
The path is constructed similarly as before, but we can choose what folder to save our work in, and must also give the file a name and supply its file extension, for example:
...
Our dplyr library in the tidyverse provides arguably the best inspection function, known as glimpse().

This gives most of the information the previous base R functions did, alongside some of the structural information from str() we saw previously.
...
This gives so much useful information in a condensed output:
