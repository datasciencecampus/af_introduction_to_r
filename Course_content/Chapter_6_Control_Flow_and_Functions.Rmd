...
For example if you wanted to see the `min()`, `max()`, `mean()`, `lenght()`  and `sd()` together. We could use the summarise function as below.
...
if we we wanted another column we could keep on copying and pasting, copying and pasting but this is not recommended as its not effiecint and could lead to errors as above did above, **did you notice the error?**

To solve this issue we can write our own function which will combine all these descriptive statistics into one function.

Following the basic structure of a function below
...
Our function name will be my_descriptive_statistics

Our input will be column, which will be the column that we are interested is seeing the descriptive statistics for.

My statements will be the calculations for the desciptive statistics
...
Often when working with data, we want to manipulate/query multiple columns of data at once.



But let's say we wanted fare (currently dollars) in pounds - how would we achieve this? 
