...
No that we know they are missing values in the data, we can use the `colSums()` function or the `summary()` function to see how many missing values we have.
...
Consider you wanted to add in a new column of the Countries in which the passengers embarked, so those who embarked in "S"(Southampton) it should say England, those who embarked in "C" (Cherbourg) should say France and those who embarked in "Q" (Quenstown) should say Ireland.
...
Another example is when we did the FizzBuzz exercise. We had the code below,
...
# The output must be the same same data type
# Here am using the TRUE to convert everything to character
# as "fizz" and "Buzz" are character data

case_when(
  (example_vector %% 3 == 0 & example_vector %% 5 == 0) ~ "FizzBuzz",
  (example_vector %% 3 == 0) ~ "Fizz",
  (example_vector %% 5 == 0) ~ "Buzz",
  TRUE ~ as.character(example_vector)
)
```
