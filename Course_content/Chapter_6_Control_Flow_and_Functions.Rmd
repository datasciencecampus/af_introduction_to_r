...
<!-- Sorry for chaning, the alignment and interference with the title when I knitted was upsetting me -JM -->
...
* Be able to use conditional statements: *if*, *else* and *else if*, as well as *case_when* and *if_else()*.
...
* Be aware of functionals using `purr`.
...
A repetitive action could be "create one of several plots".

A `for` loop can produce literally anything we want it to produce (e.g., plots, models, reports, datasets).
...
These are the most common type of loop, and the only one we are going to discuss in this course. The other type is a `while` loop.
...
* Followed by `()` brackets where we specify an index variable, `i` is commonly used but it can be anything that you want. Remember, we want to use clear and descriptive variable names. This is a place holder and will correspond to a different element for each part of the loop,
...
We start by creating an iterable, I have created a vector with **numbers 0 through 5**.
...
The **`append()`** function simply adds an element to the end of a vector. We first specify the vector we want to add to which is `result` and then specify the values that we want to add to it.  Within the function this is set to the parameter `values`.

<br>

![Image showing Control Flow](../Images/loop_example.png)
...
We already saw examples in the Vector Arithmetic section. A vectorized function is a function that will apply the same operation on each element of the vectors.
...
1. You are given a vector of measurements that are grams (g). 
   ...
   For this exercise, we actually want to convert all of these measurements to kilograms (kg). 

   **For reference, 1kg is the same as 1000 grams.**

   Using a for loop, create and append to a new vector called **my_kilograms** that contains the values            converted to kilograms.
...
for (measurement in grams) {
    ...
    my_kilograms <- append(my_kilograms, converted_kilo_grams) 
...
So, if our statement meets some criteria we may want to run different code than if it does not meet that criteria.
...
This links in with logical or boolean data types that we covered in Chapter One, Getting Started With R. 

As our logical statement has to be either `TRUE` or `FALSE`. 
...
We use comparison and logical operators to control the flow of our code.
...
Our example above shows that we can make a decision by answering "Yes" or "No" (2 branches). But often this will not be the case. We can add some more branches to our responses.
...
Finally, if none of the conditions are met the actions for the `else` condition is met and it's associated actions are run.

We separate out out different command blocks relating to each condition using curly brackets `{}` around the corresponding block.
...
In this code the block `print("Wow, it's still 5!")` will not ever run. This is because it will be skipped as it has the same condition `height == 5` as the first `if` condition.
...
# This shows the number of missing values in each column in the dataset
...
Here am going to use the mean value for `age_of_passenger`.
...
<!--Could this be clarified? I think I understand what you are intending to do, but maybe restructure the wording? -JM -->
...
imputed_titanic <- titanic %>%
  ...
  dplyr::mutate(imputed_age_of_passenger = 
                  dplyr::if_else(condition = is.na(age_of_passenger),
...
`case_when()` takes a conditional command in the same format as the first command in `if_else()`, however only the action for the TRUE condition is given, separated with a **tilde ~**.
...
```{r}
# Here I am using case_when withing the mutate function as I want to create a new column within my dataset
# My new column name is country_embarked
# To use case_when(), you specify the condition first
# Them the ~ tilde for the action when the condition is met
# We can also specify the TRUE which is what to happen if none of the set conditions are met.
...
case_when_example <- titanic %>% 
  mutate(
    country_embarked = case_when( 
      ...
      embarked == "C" ~ "France",
      embarked == "Q" ~ "Ireland",
      # This is  what to happen if none of the set
      # conditions are met
      TRUE ~ "Unknown") 
  )
...
To keep these values from getting NAs, include a final catch-all test and replacement.
...
You could also use the recode function here.

Example shown in Chapter 4 Working With Dataframes, section 4.6.2 Creating Variables Based on Existing Columns.
...
* where ever the value of the sex_of_passenger is "male" and age of passenger value is     missing should be imputed with the mean age_of_passenger of only the male passengers.

* where ever the value sex_of_passenger is "female" and age of passenger value is missing   should be imputed with the mean age_of_passenger of only the female passengers.
...
titanic_fare_level <- titanic %>% 
  dplyr::mutate(
    fare_level = dplyr::case_when(
      dplyr::between(fare, left = 0, right = 100)~ "cheap",
      dplyr::between(fare, left = 100, right = 200) ~ "fair",
      dplyr::between( fare, left = 200, right = 300) ~ "expensive",
...
# The calculating the mean

male_passenger_mean_age <- mean(filter(titanic, 
  sex_of_passenger == "male")$age_of_passenger, na.rm = TRUE)
...
female_passenger_mean_age <- mean(filter(titanic, 
  sex_of_passenger == "female")$age_of_passenger, na.rm = TRUE)


# Using case_when
# Here I am assigning it to titanic_imputed_mean_values
# using mutate to ass the new column
# casewhen specified inside the mutate function
...
Another example here is in Chapter Four - Working with Data Frames when we cleaned our column names, it was much easier to use the `janitor::clean_names()` function than to run 4 lines of code using `gsub()`
...
<!-- there was a part about non-returned values, this is not correct for R (is for python). I have changed it to my understanding of R functions. Happy to discuss -JM -->
...
Check it performs how you expect using the examples below.
...
If we we wanted another column we could keep on copying and pasting, copying and pasting but this is not recommended as its not efficient and could lead to errors as above did above.
...
<!-- Really like this example for how we can use functions -JM-->
