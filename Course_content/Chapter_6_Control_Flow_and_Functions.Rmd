...
## Inital Exercise: Loading Packages {.tabset .tabset-fade -}
...
* Be able to istinguish between a function's parameters and arguments.
...
* Then we start the loop with the word `for`,
...
We can then create our loop which doubles every value, prints it out, and stores it in the results vector.
...
for (measurement in grams) {
    ...
    kilograms <- append(kilograms, converted_kilogram) 
...
> **Please refer to the Editing and Imputation Course for details on how to best impute your missing data, this is fpr demonstration purposes only.** 
...
titanic_fare_level <- titanic %>% 
  dplyr::mutate(
    fare_level = dplyr::case_when(
      ...
      dplyr::between(fare, left = 100, right = 200) ~ "middling",
