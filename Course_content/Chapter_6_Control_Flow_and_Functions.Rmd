...
for (each_number in vector) {
     ...
     convert to kilo_grams
  
     append result to kilo_grams
}
...
This links in with logical or boolean data types that we covered in Chapter One, Getting Started With R. As our logical statement has to be either `TRUE` or `FALSE`. To evaluate these we use comparison operators which we used in Chapter 4 Working With Dataframes where we used the `filter()` function.
...
We can  put multiple `dplyr::if_else()` commands inside each other, but this makes code hard to read and is not recommended.
...
`dplyr::case_when()` takes a conditional command in the same format as the first command in `if_else()`, however only the action for the TRUE condition is given, separated with a **tilde ~**.
