...
1. You are given a vector of measurements that are in terms of millimeters (mm). For this exercise, we actually want to convert all of these measurements in terms of meters (m). For reference, 1 meters is the same as 1000 millimeters.
...
while (stop_value < 5) { 
  ...
  result <-  append(result, stop_value)
}
...
Control flow is just a techical term for something we do in everyday life: decision making. Imagine going to a supermarket to get some fruit for yourself. As you walk into the fruit section, you find your favorite fruit: oranges. When picking fruit to buy, we normally want to buy the ones that look juicy and healthy and avoid the ones that look rotten or bruised. As we pick up our fruit and inspect it, we are actually using control flow.


![Image showing Control Flow](../Images/control_flow_image.png)


This concept of guiding decisions also appears in programming, where we may need to use different code for different situations.

In order to move through this decision process, we need to answer either "Yes" or "No" to each question. A series of "Yes" answers will lead us to buy a given orange.
...
So far, we've worked under the assumption that each of the decisions in our tree had only two branches: one corresponding to TRUE and another to FALSE. There are plenty occasions where we have more than two since some decisions don't boil down to a "Yes" vs "No".
...
schools2 <- schools %>% 
  mutate(test_more = if_else(condition = `Local authority` == "Camden" , TRUE, FALSE )
    )

```

Here we have created a new column and for each rw the `if_else()` function takes the local authority and checks if it equal to the character "Camden". 

The if_else() function takes that single comparison operator condition = `Local authority` == "Camden" and vectorizes it.

Looking at help documentation for the function, we can see that it need 3 functions.
...
So far we have looked at else if to add more option to an if statement, We know about the if_else() function, but it works best with simpler conditions that evaluate to TRUE or FALSE.


Their is also a **`case_when()`** function from `dplyr` which is great for dealing with multiple complex conditions.

We could potentially nest if_else() functions in other functions, but this could make for hard-to-read code. Theree is a more elegant solution in the case_when() function.

You can think of the case_when() function as a generalization of the if_else() function, which incorporates more branches.
...
Within the case_when() function, we have a series of conditions and assignments. The first example is Total < 2000 ~ "Small. The condition Total < 2000 is separated from the assignment "Small" by the character ~ (prounounced tilde). You can think of each of these lines as shortened if branches. The last line, TRUE ~ "Medium, acts as a catch-all in case the Total amount does not fall into either of the conditions above. Another way of interpreting the TRUE ~ "Medium" line is that it is a safety net: if the Total satisfies neither the Total < 2000 or the Total > 20000 condition, then it will be "caught" by the TRUE ~ "Medium" line.



Use the case_when() function to create a new column in the recent_grads dataset called unemployment_level. This new column should have 5 levels total:
If Unemployment_rate is less than 20%, unemployment_level should be "<20%"
Between 20% and less than 40%: "20% to 40%"
Between 40% and less than 60%: "40% to 60%"
Between 60% and less than 80%: "60% to 80%"
Greater than or equal to 80%: ">=80%"
...
1. Create a new column school_size in the school dataset. If the  Total number of pupils` is greater than 1000, then school_size should take the value "Big School". If not, then school_size should take the value "Small School".
...
schools <- read_csv("Intro_to_R_Refresh/Data/schools_data.csv")


schools2 <- schools %>% 
  mutate(school_size = if_else(condition = `Total number of pupils` > 1000 , true = "Big School", false = "Small School" )
    )
```
...
We have previously used ready-made functions (from Base R or from packages), e.g. **`excel_sheets()`**, **`help()`**,**`merge()`**,**`clean_names*()`**, **`mean()`*.
...
We can take one complicated operation and wrap it up inside a function and use it wherever we need. For example, take the mean() function from base R that calculates the average value of a given vector — we could write out the average calculation by hand:
...
Nothing stops us from writing out sum(nums) / length(nums) every time we need to calculate an average and this can get tiring.
...
my_result <- function_sum_values(3, 4)
...
### Named Arguments
For more complicated functions with lots of arguments, specifying the arguments by position becomes more error-prone and harder to understand. Therefore it is often desirable to specify some or all of the arguments by name, so the order of the arguments becomes unimportant:

```{r}

function_sum_values(value_2 = 2, value_1 = 1)
...
Also, order of passing the values in is essential, as `arithmetic(a, b) != arithmetic(b, a)` (in most cases!).
