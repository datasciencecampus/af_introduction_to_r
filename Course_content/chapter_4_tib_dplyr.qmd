...
* With the tidyverse, when we use its myriad of functions, we can check manipulations on the data without overwriting thew underlying variable unless we **explicitly** ask it to do so with **<-**.

* However, with this method, we can overwrite a column for example and this will **implicitly** and permanently change the underlying variable, which we must be careful of.
...
* Or create a new one
using assignment **<-**, then it just shows us what that process will do, so we can then make the decision on whether it's what we want. 
...
You will notice that autocompletion begins to be super useful when using tidyverse libraries, creating a more efficient workflow for us.
...
We get a very large output here that is not easy to navigate to check the manipulation that has been done. We should wrap up our input in **glimpse()** to get an easier look at the result.
...
Notice that glimpse() is **written first** but **executed last**, since R evaluates function from the inside out.
...
The **select()** verb is perfect for this, as it allows for selection:
* A single column
* Multiple columns
* Sequences of columns
* De-selection of columns
and more!
...
What was done above was an oversimplification of why the pipe was created, in reality if we only needed to use one or two small functions, or simply a glimpse(), then we do not need to invoke the pipe operator. 
...
and we may even want to combine columns mathematically to create ratios, proportions etc, known as **Calculated Columns**.

We may also want to create new variables entirely, filled with a specific value or based on a condition (TRUEs and FALSES). 
...
The simplest example would be adding a constant column, which contains the same value all the way down. This effectively adds the same value to each row in the tibble. 
...
titanic_data |> 
    mutate(character_col = "two") |> # Character column filled with the string "two"
    glimpse() 
...
For example, should we wish to make the name of passenger column lower case, to avoid the sorting issue with capital letters, we can use the **str_to_lower()** function from **stringr**. 

To apply a function to an existing column, our mutate line goes as follows:
...
This has the unexpected consequence of rounding the passenger with an age of 0.1667 (around 2 months) to 0, which makes no sense given the context. As such, it may be better to round to 2 decimal places, with children below 12 months to be considered fairly. 
...
We should only overwrite or create a new variable when we are sure that this is what is required for our task.


### Calculated Columns

Let's see an example of creating a bespoke column by combining existing ones. Here we will create the family size of each person. To compute this we need:
...
These are very common and are often known as **binary flags** as they denote whether a certain condition is attained or not, which are often very useful for statistical and machine learning purposes.
...
often, with binary flags we want them in numeric form, as most models we use expect numeric coded variables. This means we need to convert TRUE and FALSE to their numeric representations of 0 and 1.
...
You may not have seen this notation before with the tilde **~**, this is known as a **formula** in R, where the right hand side is equivalent to the left hand side, and is most commonly used with linear modelling to define the equation. See this [technical article on the tilde](https://medium.com/anu-perumalsamy/what-does-mean-in-r-18cecd1b223f#:~:text='~(tilde)'%20is%20an%20operator%20that%20splits%20the%20left,the%20set%20of%20feature%20variables.&text=In%20the%20above%20example%2C%20df,the%20columns%20wages%20and%20yearsEd.) for more information.
...
1. Create a new column called "fare_dollars", which converts the fare from GBP (£) to USD (\$). The current exchange rate as of January 2024 is:
> £1 = $1.27

2. Modify your code from question 1 to round the column to 2 decimal places.
...
A very common problem in data analysis is our information being split along multiple smaller datasets, usually containing a column in common.

Much of the information we need to answer questions of interest is featured across the data, so we can **join** or **merge** them together for one cohesive dataset.
...
Our favourite manipulation library **dplyr** provides a family of **join** functions. They take the form **type_join** where the prefix "type" denotes the type of join itself. 
...
* **anti-join** - An example of a filtering join, where all data from the right tibble that does not match in the left tibble is retained. A great way to flag missing values and check for patterns. 
...
There are also Semi Joins and Anti Joins, which you can read about in Hadley Wickhams [R for Data Science Chapter 19](https://r4ds.hadley.nz/joins.html)

### Examples

Joins are best explored using examples of each type.

We will use 2 datasets that come with **dplyr**, they are very small and were made with learning how to join in mind.
...
These data sets have one common column which is **name**, so we can join them to have both band and plays in the same dataset. Let's assume that members_data is the left dataset and instruments_data is the right.
...
An anti join returns all rows from the left tibble that do not match in the right, so in this case, only Mick will be returned with his band entry, so no joining was actually performed. 

```{r}
# Anti-join to return rows that are missing in the instruments data

members_data |> 
    anti_join(y = instruments_data,
              by = "name")
...
A very common problem that occurs when joining is the matching column in both datasets doesn't match in name. 
...
The name variable is called "artist" here, which we can either rename or use the excellent tools dplyr has at its disposal for cases where we are not allowed to do so. 
...
With row binding, columns are matched by name automatically, so the positioning of them does not matter. Missing columns will be filled with NA, so we must be careful of this.

With column binding, rows are matched by position, so the tibbles we bind absolutely **must** have the same number of rows, otherwise we will get an error.

### **Binding Rows**{-}

We use the **bind_rows()** function from dplyr to accomplish this. This will widen the dataset by adding more entries.
...
Notice that the columns themselves were matched automatically, but since the instruments2 has the name variable and instruments has the artist variable, these are seen as independent, and hence filled with NAs since they don't match.

### **Binding Columns**{-}

We use the **bind_cols()** function from dplyr to accomplish this. This will lengthen the dataset by adding more variables. 
...
This may have seemed abstract so far, so why do we really want to do this in practice?
...
Another very useful function is  **union()** which will merge the data from both data frames but keep only the distinct (unique) rows. 
