...
* From R 4.1 onwards, the operator comes as standard with base R, as it has become the universal standard for functional programming in the language. It now takes the form **|>**, and this option must be turned on in the Tools --> Global Options tab. 
...
titanic_filter_example <- filter(.data = titanic, 
                                        pclass == 1 
...
titanic_filter_exercise <- filter(.data = titanic, 
                                         pclass == 2 | pclass == 3)
...
embarked_cherbourg_and_queenstown <- filter(.data = titanic,
                                                   embarked == 'C' | embarked == 'Q')

# filtering the dataset a single condition to find the survivors

survived <- filter(embarked_cherbourg_and_queenstown, 
                          survived == 1)
