...
The first structure we will look at is a vector.
...
I can now store multiple items by using the `c()` function, note elements are separated by commas 
```{r}
# Creating a vector
...
# Here I am specifying the arguements, `from`, `to` and  `by` and with the values 2, 6 and 2.
...
Here we can specify what type of vector by specifying the argument `mode` and the how long by specifying the argument `length`.
...
# Here I am specifying 2 argument, mode and length with the values 2 and 7
...
2. Create two vectors, one with numeric data and one with character data, e.g favourite_movies and lucky_numbers, see my    examples below. `(Feel free to be creative here)`.
...
The operations will scale to an larger sized vector, rather than the individual elements we have worked with thus far. 
...
We can also name our list using the `names()` function. The names() function allows us to get or set the name attribute of a data structure.
...
shopping_list[4] <- "custard creams"
...
6. Add an item called price.

7. Add a new chocolate bar.
